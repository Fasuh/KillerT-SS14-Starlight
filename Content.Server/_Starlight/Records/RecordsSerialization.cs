...
namespace Content.Server._Starlight.Records;
...
public static class RecordsSerialization
{
    ...
    private static List<RecordEntry> DeserializeEntries(
        ...
        entries
            ...
            .Select(entry => new RecordEntry(entry.Title, entry.Involved, entry.Description))
            .ToList();
    ...
    public static List<CDModel.CharacterRecordEntry> GetEntries(PlayerProvidedCharacterRecords records) =>
        ...
        records.MedicalEntries.Select(medical => ConvertEntry(medical, CDModel.DbRecordEntryType.Medical))
